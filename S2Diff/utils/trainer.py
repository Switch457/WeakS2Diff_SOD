...
class Trainer(object):
    ...
    @torch.inference_mode()
    @run_on_seed
    def val_vsod(self, model, test_data_loader, accelerator, thresholding=False, save_to=None):
        """
        validation function
        """
        global _best_mae
        ...
        maes = []
        for data in tqdm(test_data_loader, disable=not accelerator.is_main_process):
            image, depth, flow, gt, name, path = data['image'], data['depth'], data['flow'], data['gt'], data['name'], data['path']
            gt = [np.array(x, np.float32) for x in gt]
            gt = [x / x.max() + 1e-8 for x in gt]
            image = image.to(device).squeeze(1)
            depth = depth.to(device).squeeze(1)
            flow = flow.to(device).squeeze(1)
            out = self.train_val_forward_fn(model, image=image, depth=depth, flow=flow, verbose=False)
            res = out["pred"].detach().cpu()
            maes += [cal_mae(g, r, thresholding, save_to, n) for g, r, n in zip(gt, res, name)]
        # gather all the results from different processes
        accelerator.wait_for_everyone()
    ...
    def train(self):
        ...
        for epoch in range(self.cur_epoch, self.train_num_epoch):
            ...
            if (epoch + 1) % 1 == 0 or (epoch >= self.train_num_epoch * 0.7):
                ...
                mae, best_mae = self.val(self.model, self.test_loader, accelerator)  ###rgbd
                ##mae, best_mae = self.val_vsod(self.model, self.test_loader, accelerator) ###vsod
