...
class CondGaussianDiffusion(GaussianDiffusion):
    ...
    def forward(self, img, cond_img, depth_map, weak_gt, weak_mask, seg=None, extra_cond=None, *args, **kwargs):
        ...
        #weak_gt = normalize_to_neg_one_to_one(weak_gt)
        #weak_mask = normalize_to_neg_one_to_one(weak_mask)
        ...
        return self.p_losses(img, times, cond_img, depth_map, weak_gt, weak_mask, seg, extra_cond, *args, **kwargs)

    def p_losses(self, x_start, times, cond_img, depth_map, weak_gt, weak_mask, seg=None, extra_cond=None, noise=None, *args, **kwargs):
        ...
        model_out = self.model(torch.cat([x, extra_cond], dim=1), log_snr, cond_img, depth_map) # 没有extra_cond
        
        ####
        CE = torch.nn.BCELoss()
        img_size = cond_img.size(2) * cond_img.size(3) * cond_img.size(0)
        ratio = img_size / (torch.sum(weak_mask) + 1e-8)
        sal_loss2 =  ratio * CE(torch.sigmoid(model_out) * weak_mask, weak_gt * weak_mask)
        ...
        if self.loss_type == 'l2':
            ...
            return self.loss_type(model_out, target)+sal_loss2
    ...
    #######中间带噪图########
    @torch.no_grad()
    def sample(self, cond_img, depth_map, extra_cond=None, verbose=True, save_inter_dir="./results/"):
        b, c, h, w = cond_img.shape
        extra_cond = default(extra_cond, torch.zeros((b, self.extra_channels, h, w), device=self.device))
        return self.p_sample_loop((b, self.channels, self.image_size, self.image_size),
                                  cond_img, depth_map,
                                  extra_cond=extra_cond, verbose=verbose,
                                  save_inter_dir=save_inter_dir)   # <-- 新增参数

    @torch.no_grad()
    def p_sample_loop(self, shape, cond_img, depth_map, extra_cond,
                      verbose=True, save_inter_dir=None):
        self.history = []
        img = torch.randn(shape, device=self.device)
        conditioning_features, depth_features = self.model.extract_features(cond_img, depth_map)
        steps = torch.linspace(1., 0., self.num_sample_steps + 1, device=self.device)

        for i in tqdm(range(self.num_sample_steps), desc='sampling loop time step',
                      total=self.num_sample_steps, disable=not verbose):
            times = steps[i]
            times_next = steps[i + 1]
            img = self.p_sample(img, conditioning_features, depth_features, extra_cond, times, times_next)

            # 直接保存当前带噪图
            if save_inter_dir is not None:
                tmp = unnormalize_to_zero_to_one(img.clone()).clamp(0, 1)
                save_image(tmp, os.path.join(save_inter_dir, f'step_{i:04d}.png'))
    ...
    def p_mean_variance(self, x, cond, depth, extra_cond, time, time_next):
        ...
        """
        ###############保存net输出################可视化
        save_pred_dir = './results/'
        os.makedirs(save_pred_dir, exist_ok=True)
        # 逆归一化到 [0,1]
        pred_img = (pred.clone() + 1) / 2
        # 取 batch 第一张即可，或自己拼网格
        save_image(pred_img[0], os.path.join(save_pred_dir, f'pred_t{time.item():.4f}.png'))
        #########################################
        """
